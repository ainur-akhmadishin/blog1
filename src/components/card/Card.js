...
const tacg = tagList.length ? tagList.map(el=><li key = {el} className = {classes.['Card--tag']}>{el}</li>) : null

return(
<section className = {classes.Card}>
	<div className = {classes['Card--article']}>
		<div> 
			<span className = {classes['Card--title']}><Link to = {`/article/${slug}`}>{title}</Link></span>
			<input type = 'checkbox' id = 'ckeckbox' className  = {classes['Card--checkbox']}/><label htmlFor = "ckeckbox">{favoritesCount}</label>
